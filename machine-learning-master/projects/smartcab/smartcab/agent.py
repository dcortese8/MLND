...
class LearningAgent(Agent):
    ...
    def createQ(self, state):
        ...
        if state not in self.Q:
            self.Q[state] = dict((i, 0.0) for i in self.valid_actions)
...
#             self.Q[state] = self.Q.get(state, {'left': 0.0, 'right': 0.0, 'forward': 0.0, None: 0.0})
    ...
    def choose_action(self, state):
        ...
        #     action = random.choice(self.valid_actions)
        # else:
        #     maxQ, actions_max = self.get_maxQ(state)
        #     action = random.choice(actions_max)
...
def run():
    ...
    sim = Simulator(env, update_delay = 0.00, log_metrics = True, optimized = True)
